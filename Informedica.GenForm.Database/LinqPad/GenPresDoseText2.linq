...
var filter = new {Generic = "diclofenac",
				  Group = "ANALGETICA niet-morfinomimetica",
				  Route = "rect", //"iv",
				  Product = ""};
...
//DoseTexts.Where(o => o.Groups.Contains(filter.Group)).Dump();
...
foreach (var item in DoseTexts.Where(o => o.Generic == filter.Generic).Where(o => o.Groups.Contains(filter.Group)).Where(o => o.Routes.Contains(filter.Route)).Select(o => o.Products))
{
	item.Dump();
}